...
#define VALIDATE(x) ( std::isnan(x) ) ? 0 : x
    ...
    /*-----------------------------------------------------------------------*/
    {
        Mat img_l( gray_img, cv::Rect(0,0,gray_img.cols/2,gray_img.rows) );
        Mat img_r( gray_img, cv::Rect(gray_img.cols/2,0,gray_img.cols/2,gray_img.rows) );
    ...
    inline vector<double> calc_third_saliency( const Mat& gray_img )
    {
        ...
        kernel = Mat::zeros( cv::Size(10,10), CV_32FC1 );
        kernel += Scalar::all( 0.01 ); //kernel for average filtering ( 0.01=1/(10*10) )
        
        //smoothed spectrum with average filtering
        cv::filter2D( magI, smooth, CV_32FC1, kernel, cv::Point(-1,-1), 0, BORDER_REPLICATE );
        ...
        for( int a=0; a<w; ++a ) {
            for( int b=0; b<w; ++b ) {
                Mat temp = result(cv::Rect(cx*a, cy*b, cx, cy));
    ...
    /*-----------------------------------------------------------------------*/
    {
        ...
        Mat dh, dh1, dh2;
        dh1 = img( cv::Rect(1, 0, n-1, m) );
        dh2 = img( cv::Rect(0, 0, n-1, m) );
        dh = dh1 - dh2;
        ...
        Mat dv1, dv2, dv;
        dv1 = img( cv::Rect(0, 1, n, m-1) );
        dv2 = img( cv::Rect(0, 0, n, m-1) );
        dv = dv1 - dv2;
