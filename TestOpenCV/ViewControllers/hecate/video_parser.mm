...
/*-----------------------------------------------------------------------*/
{
    ...
    while( true )
    {
        ...
        if( _nfrm_total % _step_sz == 0 ) {
            if( rsz_ratio>0 )
                resize( frm, frm, cv::Size(), rsz_ratio, rsz_ratio, CV_INTER_LINEAR );
...
#pragma omp parallel for
    for( int i=0; i<_nfrm_given; i++ )
    {
        ...
        GaussianBlur( frm_gray, frm_gray, cv::Size(3,3), 0, 0 );
...
#pragma omp parallel for
    ...
    // compute edge-change-ratio (ECR)
    {
        ...
        Mat dl_elm = getStructuringElement(MORPH_CROSS,
                                           cv::Size(2*dl_sz+1, 2*dl_sz+1),
                                           cv::Point(dl_sz, dl_sz));
...
/*-----------------------------------------------------------------------*/
{
    ...
    VideoCapture vr( in_video );
    vr >> frm;
    resize( frm, frm, cv::Size(), debug_rsz_ratio, debug_rsz_ratio, CV_INTER_LINEAR );
    vr.set( CV_CAP_PROP_POS_FRAMES, 0 );
    
    cv::Size sz = frm.size();
    Mat frm_lr( sz.height, 2*sz.width, CV_8UC3 );
    Mat frm_l(frm_lr, cv::Rect(0,0,sz.width,sz.height));
    Mat frm_r(frm_lr, cv::Rect(sz.width,0,sz.width,sz.height));
