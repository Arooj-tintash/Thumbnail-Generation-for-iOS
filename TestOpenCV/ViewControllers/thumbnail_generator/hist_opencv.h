...
#ifndef hist_opencv_h
#define hist_opencv_h
...
inline void calc_color_hist( const Mat& img, Mat& hist, int nbins=128 )
{
    ...
    Mat hist0( hist, cv::Rect(0,0*nbins,1,nbins) );
...
inline void calc_edge_hist( const Mat& Gx, const Mat& Gy, Mat& hist, int nbins_ori=16, int nbins_mag=16 )
{
    ...
    Mat hist_ori( hist, cv::Rect(0,0,1,nbins_ori) );
    Mat hist_mag( hist, cv::Rect(0,nbins_ori,1,nbins_mag) );
    ...
    for(int l=0; l<level; l++) {
        for(int x=0; x<pow(2,l); x++) {
            for(int y=0; y<pow(2,l); y++) {
                ...
                int p_x = x*p_width;
                int p_y = y*p_height;
                Mat patch_Gx( Gx, cv::Rect(p_x,p_y,p_width,p_height) );
                Mat patch_Gy( Gy, cv::Rect(p_x,p_y,p_width,p_height) );
                Mat patch_hist( hist, cv::Rect(0,patch*hist_sz,1,hist_sz) );
